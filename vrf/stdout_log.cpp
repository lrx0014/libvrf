...
std::shared_ptr<Logger> NewDefaultLogger()
{
    ...
    log_handlers[static_cast<std::size_t>(LogLevel::TRACE)] = [](std::string msg) { std::cout << "[trace] " << msg << '\n'; };
    log_handlers[static_cast<std::size_t>(LogLevel::DEBUG)] = [](std::string msg) { std::cout << "[debug] " << msg << '\n'; };
    log_handlers[static_cast<std::size_t>(LogLevel::INFO)] = [](std::string msg) { std::cout << "[info] " << msg << '\n'; };
    log_handlers[static_cast<std::size_t>(LogLevel::WARN)] = [](std::string msg) { std::cerr << "[warning] " << msg << '\n'; };
    log_handlers[static_cast<std::size_t>(LogLevel::ERROR)] = [](std::string msg) { std::cerr << "[error] " << msg << '\n'; };
