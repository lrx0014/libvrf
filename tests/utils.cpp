...
BIGNUM_Guard hex_string_to_bignum(const std::string &hex_str)
{
    ...
    {
        Logger()->error("Failed to convert hex string to BIGNUM: {}", hex_str);
        return {};
    }
...
EVP_PKEY_Guard make_rsa_secret_key(Type type, const std::string &p_hex, const std::string &q_hex)
{
    ...
    {
        Logger()->error("Failed to create BIGNUMs for RSA parameters.");
        return {};
    }
    ...
    {
        Logger()->error("Unsupported VRF type for RSA key generation.");
        return {};
    }
    ...
    if (!bn_ctx.has_value())
    {
        Logger()->error("Failed to allocate BN_CTX.");
        return {};
    }
    ...
    if (!e_bn.has_value() || !n.has_value() || !p_minus_1.has_value() || !q_minus_1.has_value() || !gcd.has_value() ||
        ...
        !iqmp.has_value())
    {
        Logger()->error("Failed to allocate BIGNUMs for RSA key generation.");
        return {};
    }

    if (1 != BN_set_word(e_bn.get(), params.e))
    {
        Logger()->error("Failed to set public exponent e.");
        return {};
    }

    if (1 != BN_mul(n.get(), p.get(), q.get(), bn_ctx.get()))
    {
        Logger()->error("Failed to compute RSA modulus n = p*q.");
        return {};
    }
    ...
    {
        Logger()->error("Failed to compute p-1 and q-1.");
        return {};
    }

    if (1 != BN_gcd(gcd.get(), p_minus_1.get(), q_minus_1.get(), bn_ctx.get()))
    {
        Logger()->error("Failed to compute gcd(p-1, q-1).");
        return {};
    }
    ...
    {
        Logger()->error("Failed to divide (p-1) by gcd for lcm.");
        return {};
    }
    ...
    {
        Logger()->error("Failed to compute lcm(p-1, q-1).");
        return {};
    }
    ...
    {
        Logger()->error("Public exponent e is not invertible modulo λ(n). Bad RSA parameters?");
        return {};
    }
    ...
    {
        Logger()->error("Failed to compute CRT exponents dmp1/dmq1.");
        return {};
    }
    ...
    {
        Logger()->error("Failed to compute iqmp = q^{-1} mod p.");
        return {};
    }
    ...
    // Sanity check: ensure gcd(e, p-1) == 1 and gcd(e, q-1) == 1
    {
        ...
        if (!ge.has_value() || !gq.has_value() || 1 != BN_gcd(ge.get(), e_bn.get(), p_minus_1.get(), bn_ctx.get()) ||
            ...
            !BN_is_one(gq.get()))
        {
            Logger()->error("Public exponent e is not coprime with p-1 and/or q-1.");
            return {};
        }
    }
    ...
    if (nullptr == bld)
    {
        Logger()->error("OSSL_PARAM_BLD_new failed.");
        return {};
    }
    ...
    if (!ok)
    {
        ...
        Logger()->error("Failed to push RSA parameters into OSSL_PARAM_BLD.");
        return {};
    }
    ...
    {
        OSSL_PARAM_BLD_free(bld);
        Logger()->error("OSSL_PARAM_BLD_to_param failed.");
        return {};
    }
    ...
    if (1 != EVP_PKEY_fromdata_init(pctx.get()))
    {
        ...
        OSSL_PARAM_BLD_free(bld);
        Logger()->error("EVP_PKEY_fromdata_init failed.");
        return {};
    }
    ...
    if (1 != EVP_PKEY_fromdata(pctx.get(), pkey.free_and_get_addr(), EVP_PKEY_KEYPAIR, paramlist))
    {
        OSSL_PARAM_free(paramlist);
        OSSL_PARAM_BLD_free(bld);
        Logger()->error("EVP_PKEY_fromdata failed to create RSA EVP_PKEY.");
        return {};
    }
    ...
    // Check the created key.
    {
        ...
        if (!ck.has_value())
        {
            Logger()->error("Failed to create EVP_PKEY_CTX for checking constructed RSA key.");
            return {};
        }

        if (1 != EVP_PKEY_check(ck.get()))
        {
            Logger()->error("Constructed RSA EVP_PKEY failed EVP_PKEY_check.");
            return {};
        }
    }
...
std::unique_ptr<SecretKey> make_rsa_vrf_secret_key(Type type, const std::string &p_hex, const std::string &q_hex)
{
    ...
    if (!pkey.has_value())
    {
        Logger()->error("Failed to create RSA EVP_PKEY for VRF secret key.");
        return nullptr;
    }
    ...
    {
        Logger()->error("Failed to create RSA_SK_Guard for VRF secret key.");
        return nullptr;
    }
    ...
    {
        Logger()->error("Failed to create RSA VRF secret key from EVP_PKEY.");
        return nullptr;
    }
...
std::unique_ptr<SecretKey> make_ec_vrf_secret_key(Type type, const std::string &sk_hex)
{
    ...
    {
        Logger()->error("Failed to convert hex string to EC scalar for VRF secret key.");
        return nullptr;
    }
    ...
    if (!sk->is_initialized())
    {
        Logger()->error("Failed to create EC VRF secret key from scalar.");
        return nullptr;
    }
...
std::vector<std::byte> parse_hex_bytes(std::string_view s)
{
    if (s.length() % 2 != 0)
    {
        Logger()->error("Hex string has odd length: {}", s.size());
        return {};
    }
    ...
    if (hi != -1)
    {
        Logger()->error("Hex string has odd number of hex digits after filtering: {}", s.size());
        return {};
    }
...
RSA_VRF_TestVectorParams get_rsa_vrf_test_vector_params(Type type)
{
    ...
    default:
        Logger()->error("No test vector parameters defined for VRF type: {}", type_to_string(type));
        return {};
    }
}
